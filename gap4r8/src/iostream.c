...
void ChildStatusChanged( int whichsig )
{
#ifndef _BEC_
  UInt i;
  int status; __wait_status_ptr_t tmp_status;
  int retcode;
  assert(whichsig == SIGCHLD);
  HashLock(PtyIOStreams);
  tmp_status.__int_ptr = &status;
  for (i = 0; i < MAX_PTYS; i++) {
      if (PtyIOStreams[i].inuse) {
		  retcode = waitpid( PtyIOStreams[i].childPID, tmp_status, WNOHANG | WUNTRACED );
  ...
  do {
      retcode = waitpid( -1, tmp_status, WNOHANG);
...
/*UInt count; */
  ...
  retcode = waitpid(PtyIOStreams[pty].childPID, tmp_status, 0);
  FreeStream(pty);
  return 0;
#else
	assert(0);
